...
- Node.js `>=20.20.0`
- 已安装至少一种 provider CLI：`codex`、`claude`、`gemini`、`opencode`、`openclaw`、`nanobot`、`hermes`
- `serverUrl`
- `daemonToken`
...
- `gemini`：继续沿用 CLI one-shot 调用
- `opencode`：当前走 `opencode run --format json`，适合已有本地 repo/workDir 的 one-shot 执行
...
- `opencode` / `openclaw` / `nanobot` 各自还有更长期驻留的 server/gateway/serve 形态；当前 daemon 集成优先接通的是最容易嵌入现有 task/workDir 模型的 headless CLI 路径
...
`agent-router` 是同包发布的轻量跨 harness CLI，用来直接验证 Claude Code、Codex CLI、OpenClaw、Hermes Agent 的原生 headless 调用与统一结果 contract。daemon 的 Claude Code、Codex CLI、OpenClaw、Hermes Agent task execution 已经通过 AgentRouter 执行；AgentSpace task queue、runtime-output、gws、workspace skills 和 Web UI 仍由 daemon 外层流程处理。Gemini、OpenCode、NanoBot 暂时保留旧 provider runtime 路径。
